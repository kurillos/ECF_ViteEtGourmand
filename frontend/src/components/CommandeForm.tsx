...
const CommandeForm = () => {
    ...
    const fraisLivraison = formData.ville.toLowerCase() === 'bordeaux' 
        ? 0 
        : 5 + (10 * 0.59); // 10km par défaut pour le test
    
    const total = prixBase - remise + fraisLivraison;

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        alert(`Commande confirmée. Un mail a été envoyé à ${clientEmail} !`);
        navigate('/');
    };

    return (
        <div className="max-w-3xl mx-auto p-10 bg-white rounded-[2.5rem] shadow-2xl mt-10 mb-10 border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="p-6 bg-gray-50 rounded-3xl">
                    <p className="text-sm"><strong>Email client :</strong> {clientEmail}</p>
                </div>
               ...
               <div>
                    <label className="text-xs font-bold text-gray-400 uppercase ml-2">
                        Nombre de personnes (Min: {menu.nb_personnes})
                    </label>
                    <input 
                        type="number" 
                        min={menu.nb_personnes} 
                        value={nbPers} 
                        onChange={(e) => {
                        const val = parseInt(e.target.value);
                        setNbPers(isNaN(val) ? menu.nb_personnes : val);
                        }} 
                        className="w-full p-4 bg-gray-100 rounded-2xl text-xl font-bold" 
                    />
                </div>
